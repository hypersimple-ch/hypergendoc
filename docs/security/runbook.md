...
A recovery must use PostgreSQL and the Git volume from the same consistent backup set. Preserve object storage separately for uploaded logos, fonts, and template media. Document source, generated HTML, PDFs, and render records are not object-store artifacts; PDFs are rendered in memory only.
